...
if __name__ == '__main__':
    ...
    with open("Android.mk", "w") as header_file:
        ...
        file_comment += '# See vllf_makefile_generator.py for modifications\n'
        ...
        # Get list of subdirectories in state_trackers (dest) dir
        state_tracker_path = "%s/*/" % vllf_dir
        state_tracker_dirs = glob(state_tracker_path)

        # Makefile source common to all state_trackers
        contents =  '#pragma once\n\n'
        contents += 'LOCAL_PATH := $(call my-dir)\n'
        contents += 'SRC_DIR := ../../..\n'
        contents += 'LAYER_DIR := .\n'

        # Output makefile target section for each state_tracker in dest sub-directory
        for state_tracker_path in state_tracker_dirs:
            state_tracker = os.path.basename(os.path.normpath(state_tracker_path))
            contents += '\n'
            contents += 'include $(CLEAR_VARS)\n'
            contents += 'LOCAL_MODULE := VkLayer_%s\n' % state_tracker
            contents += 'LOCAL_SRC_FILES += $(LAYER_DIR)/layer_framework.cpp\n'
            contents += 'LOCAL_SRC_FILES += $(SRC_DIR)/layers/vk_layer_table.cpp\n'
            # Add *.cpp files (if any) to makefile dependencies
            for path, subdirs, files in os.walk(state_tracker):
                for file in files:
                    if '.cpp' in file:
                        contents += 'LOCAL_SRC_FILES += $(LOCAL_PATH)/$(SRC_DIR)/layers/state_trackers/%s/%s\n' & (state_tracker, file)
            ...
            contents += 'LOCAL_C_INCLUDES += $(LOCAL_PATH)/$(SRC_DIR)/layers/state_trackers/%s\n' % state_tracker
