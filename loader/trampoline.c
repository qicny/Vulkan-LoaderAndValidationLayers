...
LOADER_EXPORT VKAPI_ATTR VkResult vkEnumerateInstanceExtensionProperties(
    ...
    for (uint32_t i = 0; i < layers.count; ++i) {
        ...
        if (pfn == NULL) {
            loader_log(NULL, VK_DEBUG_REPORT_WARNING_BIT_EXT, 0, "%s: Unable to resolve symbol \"%s\" in implicit layer library \"%s\"",
                __FUNCTION__, layers.list[i].pre_instance_functions.enumerate_instance_extension_properties, layers.list[i].lib_name);
            continue;
        }
...
LOADER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceLayerProperties(
    ...
    for (uint32_t i = 0; i < layers.count; ++i) {
        ...
        if (pfn == NULL) {
            loader_log(NULL, VK_DEBUG_REPORT_WARNING_BIT_EXT, 0, "%s: Unable to resolve symbol \"%s\" in implicit layer library \"%s\"",
                __FUNCTION__, layers.list[i].pre_instance_functions.enumerate_instance_layer_properties, layers.list[i].lib_name);
            continue;
        }
